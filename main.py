...
from data.watched_videos import WatchedVideos
...
def open_video(videofilename):
    ...
    created_data = video_information.created_data.date()
    subscribes_count = db_sess.query(PeopleSubscribes).filter(PeopleSubscribes.channel_id == channel_id).count()
    watching_count = db_sess.query(WatchedVideos).filter(WatchedVideos.video_id == videoid).count()
    likes_count, dislikes_count = get_likes_and_dislikes_count(videoid)

    watching_status = db_sess.query(WatchedVideos).filter((WatchedVideos.video_id == videoid),
                                                          (WatchedVideos.person_id == current_user.id)).first()
    if not watching_status:
        watching_video(videoid, current_user.id)
...
def watching_video(videoid, user_id):
    db_sess = db_session.create_session()

    watched_video = WatchedVideos()

    watched_video.video_id = videoid
    watched_video.person_id = user_id

    video = db_sess.query(OpenAccessVideos).filter(OpenAccessVideos.id == videoid).first()
    video.watches += 1

    db_sess.add(watched_video)
    db_sess.commit()
...
def channel_description(channel_id):  # открыть описание канала
    ...
    subscribers_count = db_sess.query(PeopleSubscribes).filter(PeopleSubscribes.channel_id == channel_id).count()

    all_videos = db_sess.query(OpenAccessVideos).filter(OpenAccessVideos.author_id == channel_id).all()
    all_videos_watches = 0
    for video in all_videos:
        all_videos_watches += video.watches
    ...
    channel_info = {
        ...
        "vk": vk,
        "inst": inst,
        "mail_for_cooperation": mail_for_cooperation,
        "all_videos_watches": all_videos_watches,
        "subscribers_count": subscribers_count
    }

    return render_template("Channel_description.html", channel_id=int(channel_id),
                           channel_name=channel_name, active_buttons_classes=active_buttons_classes, channel_info=channel_info)
